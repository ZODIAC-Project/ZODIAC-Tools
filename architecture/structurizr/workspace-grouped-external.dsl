    ...
    model {
        ...
        zodiac = softwareSystem "ZODIAC Platform" "Purpose-aware AI and MQTT platform for orchestration, live streams, agent management, and RAG-backed subsidy workflows." {
            ...
            group "RAG Runtime" {
                ...
                ragStore = container "Chroma Data Store" "Embedded persistent vector store used by the RAG service; stores embeddings and purpose-tagged metadata." "ChromaDB, local volume" "Database, ExternalDependency"
            }

            group "Broker Runtime" {
                broker = container "HiveZODIAC Broker" "HiveMQ-based MQTT broker exposing HTTP, MQTT, and WebSocket endpoints." "HiveMQ" "ExternalDependency"
